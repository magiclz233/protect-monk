...
**蝙蝠妖 move：**
```text
生成蝙蝠妖 Q版西游塔防敌人 move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。深紫色翅膀的小型蝙蝠妖，Q版圆润蝠翼，低空扑翅飞行前进，翅膀上下扇动，深紫妖气微散。角色位置稳定。无文字/水印。
```

**蝙蝠妖 hit：**
```text
生成蝙蝠妖 Q版西游塔防敌人 hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 320×80。深紫蝙蝠妖受击翅膀收缩→身体后翻→翅膀展开恢复，不能血腥。角色位置稳定。无文字/水印。
```

**蝙蝠妖 death：**
```text
生成蝙蝠妖 Q版西游塔防敌人 death 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。蝙蝠妖翅膀扭曲→坠落地面→化为深紫色烟雾消散，不能血腥。角色位置稳定。无文字/水印。
```

**巡山妖 move：**
```text
生成巡山妖 Q版西游塔防敌人 move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。棕红色野妖，山野小妖造型，手持简陋木棍，东张西望巡视前进，步伐鬼祟。角色位置稳定。无文字/水印。
```

**巡山妖 hit：**
```text
生成巡山妖 Q版西游塔防敌人 hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 320×80。棕红巡山妖受击后仰→木棍脱手→惊惶表情，不能血腥。角色位置稳定。无文字/水印。
```

**巡山妖 death：**
```text
生成巡山妖 Q版西游塔防敌人 death 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。巡山妖踉跄→倒地→缩成一团→化为棕红烟尘消散，不能血腥。角色位置稳定。无文字/水印。
```
...
**青狮 idle：**
```text
生成青狮 Q版西游塔防 BOSS idle 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 512×128。青绿色巨型狮妖，厚重鬃毛如火焰般蓬松，青绿色皮肤，锐利兽爪，狮尾慢摇，威严雄壮的待机姿态。角色位置稳定。无文字/水印。
```

**青狮 move：**
```text
生成青狮 Q版西游塔防 BOSS move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 768×128。青绿巨狮，四足沉稳踱步前进，鬃毛随步伐飘动，每步威严有力，青绿色妖气尾随。角色位置稳定。无文字/水印。
```

**青狮 attack：**
```text
生成青狮 Q版西游塔防 BOSS attack 动作，横向连续 8 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 1024×128。青绿巨狮，巨口张开→深吸气→发出青色恐惧咆哮声波→青色声波光环向前扩散→合口，恐惧咆哮震慑感。角色位置稳定。无文字/水印。
```

**青狮 hit：**
```text
生成青狮 Q版西游塔防 BOSS hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 512×128。青狮受击鬃毛炸起→身体后缩怒吼→恢复攻击姿态。角色位置稳定。无文字/水印。
```

**青狮 death：**
```text
生成青狮 Q版西游塔防 BOSS death 动作，横向连续 8 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 1024×128。青狮鬃毛逐一黯淡→庞大身躯倒地→青色妖气从身体溢出→身体缩小→化为青绿烟尘，不能血腥。角色位置稳定。无文字/水印。
```

**青狮 spawn：**
```text
生成青狮 Q版西游塔防 BOSS spawn 出场动画，横向连续 8 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 1024×128。青色旋风从地面升起→风中狮影逐渐清晰→青狮从旋风中跃出落地→仰天咆哮→鬃毛炸开，BOSS登场威猛感。角色位置稳定。无文字/水印。
```

**白象 idle：**
```text
生成白象 Q版西游塔防 BOSS idle 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 512×128。灰白色巨型战象，身披厚重铠甲，长鼻粗壮，象牙尖锐，灰白重甲防御感十足，如山般稳固待机姿态。角色位置稳定。无文字/水印。
```

**白象 move：**
```text
生成白象 Q版西游塔防 BOSS move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 768×128。灰白巨象，四足如柱沉重踏步前进，重甲碰撞声感，每一步地面剧烈震动，不可阻挡的推进感。角色位置稳定。无文字/水印。
```

**白象 attack：**
```text
生成白象 Q版西游塔防 BOSS attack 动作，横向连续 8 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 1024×128。灰白巨象，前足高高抬起→猛力向下践踏→地面裂纹以巨象为中心向外扩散→灰色冲击波环形爆发→收足，范围践踏毁灭感。角色位置稳定。无文字/水印。
```

**白象 hit：**
```text
生成白象 Q版西游塔防 BOSS hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 512×128。灰白巨象受击重甲震动闷响→长鼻扬起→稳回防御姿态。角色位置稳定。无文字/水印。
```

**白象 death：**
```text
生成白象 Q版西游塔防 BOSS death 动作，横向连续 8 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 1024×128。白象重甲块块脱落→庞大身躯缓缓跪下→轰然侧倒→地面巨震→化为灰白石粉消散，不能血腥。角色位置稳定。无文字/水印。
```

**白象 spawn：**
```text
生成白象 Q版西游塔防 BOSS spawn 出场动画，横向连续 8 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 1024×128。地面裂开升起巨大石台→石台碎裂→白象从碎石中昂首走出→重甲一块块飞附身上→长鼻高举，BOSS登场沉重感。角色位置稳定。无文字/水印。
```
...
**恐惧咆哮声波：**
```text
生成青狮BOSS技能「恐惧咆哮声波」特效，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 768×128。青狮巨口张开→青色声波光环从口中发出→声波环向前扇形扩散→声波环逐渐变大变淡→声波覆盖前方区域→光环消失。无文字/水印。
```

**高速冲锋拖尾：**
```text
生成青狮BOSS技能「高速冲锋拖尾」特效，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 128×128，整张 768×128。青狮身体前倾→瞬间高速前冲→身后留下青色高速拖尾气流线→拖尾从短变长→气流线逐渐淡化→冲锋结束。无文字/水印。
```

**践踏地面裂纹冲击：**
```text
生成白象BOSS技能「践踏地面裂纹冲击」特效，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 256×256，整张 1536×256。白象前足高高抬起→猛力践踏地面→地面裂纹从践踏点向外急速扩散→裂纹带灰色冲击波→裂纹覆盖圆形区域→地面震动渐止。无文字/水印。
```
