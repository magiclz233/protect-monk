...
敌人静态：{中文名}.png                例：山精.png、黄风怪.png、黑熊精.png
敌人动画：{中文名}_{动作}.png            例：山精_move.png
敌人动画帧序列：{中文名}_{动作}_{帧号}.png   例：山精_move_01.png
...
| enemyId | 名称 | 视觉特征 |
|---------|------|---------|
| xiaoyao_1 | 山精 | 棕绿，山林小妖怪，持短刀 |
| xiaoyao_2 | 虎妖 | 橙棕虎纹，猛兽幼体 |
| xiaoyao_3 | 骷髅妖 | 白骨轮廓，灰白，亡骨主题 |
| xiaoyao_4 | 白骨兵 | 白骨持刀，白虎岭士兵感 |
| xiaoyao_5 | 水妖 | 蓝绿，水纹感 |
...
**提示词：**
```text
生成一张透明背景 2D 静态 sprite sheet，10 个《守护唐僧》Q版西游普通敌人横向排列。每格 80×80，整张 800×80。统一45度俯视移动方向。依次：山精（棕绿山林杂兵）、虎妖（橙棕虎纹）、骷髅妖（灰白骨骼）、白骨兵（白骨持刀）、水妖（蓝绿水纹）、虾兵（蓝灰甲壳长枪）、蟹将（灰蓝大钳甲壳）、火妖（红橙火焰）、熔岩怪（暗红岩石橙裂纹）、狮驼小妖（暗绿兽面杂兵）。Q版但有威胁感，轮廓清晰。透明背景，无文字/水印。
```
...
|---------|------|------|---------|------|
| elite_huwei | 虎先锋 | Ch1 | 橙棕虎纹，敏捷虎妖 | 96×96 |
...
**山精 move：**
```text
生成山精 Q版西游塔防敌人 move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。棕绿色山林小妖怪，Q版小体型，手持短刀，灵活碎步小跑前进，草木色妖气微散。角色位置稳定。无文字/水印。
```

**山精 hit：**
```text
生成山精 Q版西游塔防敌人 hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 320×80。受击闪白、后仰抖动，不能血腥。角色位置稳定。无文字/水印。
```

**山精 death：**
```text
生成山精 Q版西游塔防敌人 death 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。从受击→倒下→化为烟尘消散，不能血腥。角色位置稳定。无文字/水印。
```
...
**虎妖 move：**
```text
生成虎妖 Q版西游塔防敌人 move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。橙棕色虎纹小型虎妖，Q版幼虎猛兽造型，四足小跑前进，虎尾摆动，橙棕妖气微散。角色位置稳定。无文字/水印。
```

**虎妖 hit：**
```text
生成虎妖 Q版西游塔防敌人 hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 320×80。橙棕虎妖受击身体后缩→虎尾炸毛竖立→虎纹短暂暗下→恢复低吼警戒姿态，不能血腥。角色位置稳定。无文字/水印。
```

**虎妖 death：**
```text
生成虎妖 Q版西游塔防敌人 death 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。虎妖踉跄→侧倒→虎纹逐渐黯淡→身体化为橙棕色烟尘飘散，不能血腥。角色位置稳定。无文字/水印。
```

**白骨兵 move：**
```text
生成白骨兵 Q版西游塔防敌人 move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。白骨持刀的白虎岭士兵，Q版白骨身躯披残破盔甲，手持骨刀，骨架步伐整齐前进，白骨寒气微散。角色位置稳定。无文字/水印。
```

**白骨兵 hit：**
```text
生成白骨兵 Q版西游塔防敌人 hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 320×80。白骨兵受击骨架震动→骨刀微落→身体后缩→骨架重新站稳，不能血腥。角色位置稳定。无文字/水印。
```

**白骨兵 death：**
```text
生成白骨兵 Q版西游塔防敌人 death 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 80×80，整张 480×80。白骨兵骨架崩散→骨刀断裂坠地→白骨逐块碎裂→化为灰白骨粉飘散，不能血腥。角色位置稳定。无文字/水印。
```
...
**虎先锋 idle：**
```text
生成虎先锋 Q版西游塔防精英敌人 idle 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 96×96，整张 384×96。橙棕色虎纹精英虎妖，体型比普通虎妖更大更威猛，虎纹鲜艳，虎爪锐利，虎尾慢摇，低伏蓄势的敏捷型待机姿态。角色位置稳定。无文字/水印。
```

**虎先锋 move：**
```text
生成虎先锋 Q版西游塔防精英敌人 move 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 96×96，整张 576×96。橙棕虎纹精英虎妖，四足矫健奔跑前进，虎尾在身后甩动保持平衡，每一步轻捷有力，虎纹妖气微散。角色位置稳定。无文字/水印。
```

**虎先锋 attack：**
```text
生成虎先锋 Q版西游塔防精英敌人 attack 动作，横向连续 6 帧 PNG sprite sheet，透明背景。每帧 96×96，整张 576×96。橙棕虎纹精英虎妖，虎爪蓄力→迅猛向前挥出利爪→橙色爪痕光弧飞出一收回，虎躯前扑爆发感。角色位置稳定。无文字/水印。
```

**虎先锋 hit：**
```text
生成虎先锋 Q版西游塔防精英敌人 hit 动作，横向连续 4 帧 PNG sprite sheet，透明背景。每帧 96×96，整张 384×96。橙棕虎纹虎妖受击虎躯一震→身体后缩怒嚎→虎纹短暂暗下→重新伏低恢复戒备，不能血腥。角色位置稳定。无文字/水印。
```
